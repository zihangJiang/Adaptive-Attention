...
IMG_CACHE = {}
class StanfordDogGenerator(object):
    image_url = 'http://vision.stanford.edu/aditya86/ImageNetDogs/images.tar'
    annos_url = 'http://vision.stanford.edu/aditya86/ImageNetDogs/annotation.tar'
    def __init__(self, input_args):
        self.input_args = input_args
        if self.input_args.tar_dir is not None:
            print('Untarring StanfordDog image package')
            self.image_dir = '../dataset/StanfordDog/Images'
    ...
    def process_original_files(self):
        ...
        assert len(all_classes)==120
        self.processed_img_dir = '../dataset/StanfordDog/processed_images'
        split_lists = ['train', 'val', 'test']
        split_numbers = {'train':70, 'val':20, 'test':30}
        ...
        for stage in split_lists:
            ...
            with open('../dataset/StanfordDog/csv_files/{}.csv'.format(stage)) as csvfile:
        ...
        # classes = {}
        # classes['train'] = all_classes[:70]
        # classes['val'] = all_classes[70:90]
        # classes['test'] = all_classes[90:]
...
class StanfordDogDataset(data.Dataset):
    raw_folder = 'images'
    processed_folder = 'processed_images'
    def __init__(self, mode='train', root='../StanfordDog', transform=None, target_transform=None):
        '''
        ...
        '''
        super(StanfordDogDataset, self).__init__()
        self.root = root
    ...
    def __getitem__(self, idx):
        ...
        # x.save('../exp/visulize/'+file_path.split('/')[-1])
        #x = self.x[idx]
        if self.transform:
...
if __name__=='__main__':
    ...
    parser.add_argument('--tar_dir',  type=str, help='tar StanfordDog dir')
    parser.add_argument('--image_dir',  type=str, default = '../dataset/StanfordDog/Images', help='untar StanfordDog dir')
    args = parser.parse_args()
    dataset_generator = StanfordDogGenerator(args)
